...
use gitp2p_federation::set_relay_enabled;
use gitp2p_sync::sync::discover_filesystem;
...
fn cross_domain_sync_fixture() {
    let root = std::env::temp_dir().join(format!(
        "gitp2p-v5-sync-{}",
        gitp2p_core::util::stable_id("global-sync")
    ));
    ...
    let session = gitp2p_federation::global_sync(&app_a, Some("repo"), &domain_b.id, false, false).unwrap();
    assert_eq!(session.phase, "complete");

    let paths = gitp2p_federation::inspect_sync_path(&app_a, Some(&session.id)).unwrap();
