...
use gitp2p_content::{create_structured_bundle, validate_bundle, ExportOptions, export_bundle};
...
fn structured_bundle_roundtrip() {
    let root = std::env::temp_dir().join(format!(
        "gitp2p-v3-{}",
        gitp2p_core::util::stable_id("bundle")
    ));
...
fn incremental_bundle_export() {
    let root = std::env::temp_dir().join(format!(
        "gitp2p-v3-inc-{}",
        gitp2p_core::util::stable_id("inc")
    ));
    let _ = fs::remove_dir_all(&root);
    fs::create_dir_all(&root).unwrap();
