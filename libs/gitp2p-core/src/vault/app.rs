...
use crate::metadata::util::{create_dir_all, default_home};
use crate::trust::ensure_identity;
...
impl App {
    ...
    pub fn all_peers(&self) -> Result<Vec<Peer>> {
        ...
        for entry in fs::read_dir(dir)? {
            let entry = entry?;
            if entry.file_type()?.is_file() {
                peers.push(crate::trust::read_peer(&entry.path())?);
            }
        }
    ...
    pub fn all_sessions(&self) -> Result<Vec<crate::Session>> {
    ...
    pub fn find_session(&self, id: &str) -> Result<crate::Session> {
...
pub fn read_session(path: &Path) -> Result<crate::Session> {
    let map = read_kv(path)?;
    Ok(crate::Session {
        id: field(&map, "id")?,
...
pub fn write_session(app: &App, session: &crate::Session) -> Result<()> {
    write_kv(
