use crate::{Checkpoint, Repo, RepoAction, Result};
use crate::git::{ensure_git_repo, git, git_output};
use crate::metadata::util::{append_log, compact_timestamp, timestamp};
use crate::trust::{
    enforce_repo_action, merged_policy, sign_checkpoint, zone_requires_policy_approval,
};
use super::app::{write_checkpoint, write_repo};
use super::layout::mirror_path;
...
pub fn create_checkpoint(
    ...
    if !repo.latest_checkpoint.is_empty() {
        if let Ok((_, _, existing)) = app.find_checkpoint(&repo.latest_checkpoint) {
...
pub fn copy_checkpoint_if_missing(vault: &crate::Vault, checkpoint: &Checkpoint) -> Result<()> {
    let path = vault
        .path
        .join("metadata")
        .join("checkpoints")
