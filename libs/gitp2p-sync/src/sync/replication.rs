...
use gitp2p_core::{Checkpoint, Peer, Repo, Result, Session, SessionPhase};
use gitp2p_core::git::git;
use gitp2p_core::util::{append_replication_log, listen_port, timestamp, transport_mode};
use gitp2p_core::trust::{
...
use gitp2p_core::trust::validate_peer_identity;
use gitp2p_core::{
...
fn new_session(peer: &Peer, repo: &Repo, checkpoint: &Checkpoint, requires_approval: bool) -> Session {
    Session {
        id: format!(
            "session-{}",
            gitp2p_core::util::stable_id(&format!("{}:{}:{}", peer.id, repo.id, timestamp()))
        ),
...
pub fn replicate_filesystem(
    ...
    gitp2p_core::app::write_repo(&remote_vault, &remote_repo)?;
    ...
    for vault in app.all_vaults()? {
        ...
        for entry in std::fs::read_dir(dir)? {
            ...
            if entry.file_type()?.is_file() {
                ...
                let pid = gitp2p_core::optional_field(&map, "peer_id");
                if peer_id.map(|id| id == pid).unwrap_or(true) {
                    history.push((
                        pid,
                        gitp2p_core::optional_field(&map, "repo_id"),
                        gitp2p_core::optional_field(&map, "sync_history"),
                    ));
                }
            }
        }
...
pub fn discover_filesystem(app: &App, homes: &[PathBuf]) -> Result<Vec<Peer>> {
    ...
    for home in homes {
        ...
        let remote_identity = gitp2p_core::trust::load_identity(&identity_path)?;
