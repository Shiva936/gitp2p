...
use gitp2p_core::{Peer, Result};
use gitp2p_core::util::{listen_port, timestamp};
...
fn peer_from_service(
    ...
    let peer_id = props
        ...
        .filter(|v| !v.is_empty())
        .ok_or_else(|| gitp2p_core::AppError::new("mdns peer missing peer_id"))?;
...
async fn listen_peers_async(app: &App) -> Result<()> {
    ...
    let quic = tokio::spawn(async move {
        let listener_app = gitp2p_core::App::with_home(app_home);
