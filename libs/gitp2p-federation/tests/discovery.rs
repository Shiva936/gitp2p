...
use gitp2p_federation::{create_gateway, exchange_routes};
use gitp2p_federation::discovery::{discover_domains, discover_gateways};
...
fn global_discovery_from_peering_fixture() {
    let root = std::env::temp_dir().join(format!(
        "gitp2p-v5-discovery-{}",
        gitp2p_core::util::stable_id("discovery-test")
    ));
