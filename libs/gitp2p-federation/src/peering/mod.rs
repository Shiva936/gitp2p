...
use crate::{ensure_federation_layout, peering_dir};
use crate::{find_gateway, list_gateways};
use gitp2p_core::identity::peering_id;
use gitp2p_core::{
...
pub fn connect_domains(
    ...
    let local_gw = match local_gateway_id {
        ...
        None => list_gateways(app)?
            ...
            .ok_or_else(|| gitp2p_core::AppError::new("no local gateway found"))?,
    };
...
pub fn find_peering(app: &App, remote_domain: &str) -> Result<Peering> {
    list_peerings(app)?
        ...
        .ok_or_else(|| {
            gitp2p_core::AppError::new(format!("peering with '{remote_domain}' not found"))
        })
}
