use gitp2p_metadata::{Checkpoint, Peer, Repo, Result, Session};
...
async fn quic_replicate(
    ...
    let remote_mirror = if peer.home.exists() {
        gitp2p_vault::ensure_remote_vault(
            &gitp2p_vault::App::with_home(peer.home.clone()),
            &vault,
        )
        .ok()
        .map(|remote_vault| gitp2p_vault::layout::mirror_path(&remote_vault, repo))
    } else {
        None
    };
    ...
    gitp2p_vault::write_session(app, session)?;
