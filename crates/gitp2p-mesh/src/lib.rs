use gitp2p_metadata::{Result, Session, SyncPath};
use gitp2p_metadata::util::{create_dir_all, timestamp};
use gitp2p_relay::forward_propagation;
use gitp2p_routing::{build_global_route, cache_global_route, discover_routes, select_route, validate_route};
use gitp2p_gateway::sync_forward;
use gitp2p_sync::sync_to_peer;
...
pub fn mesh_sync(
    ...
    let route = select_route(&routes).ok_or_else(|| {
        gitp2p_metadata::AppError::new(format!("no route to destination '{destination}'"))
    })?;
    ...
    last_session.ok_or_else(|| gitp2p_metadata::AppError::new("mesh sync produced no session"))
}
...
pub fn multi_hop_reconcile(app: &App, repo_id: &str) -> Result<Vec<String>> {
    ...
    for vault in app.all_vaults()? {
        ...
        for entry in std::fs::read_dir(dir)? {
            ...
            if gitp2p_metadata::optional_field(&map, "repo_id") != repo_id {
                continue;
            }
            let cp = gitp2p_metadata::optional_field(&map, "checkpoint_id");
...
pub fn inspect_sync_path(app: &App, session_id: Option<&str>) -> Result<Vec<SyncPath>> {
    ...
    for entry in std::fs::read_dir(dir)? {
        let map = gitp2p_metadata::read_kv(&entry?.path())?;
        let sid = gitp2p_metadata::optional_field(&map, "session_id");
        if session_id.is_some_and(|s| s != sid) {
            continue;
        }
        paths.push(SyncPath {
            session_id: sid,
            repo_id: gitp2p_metadata::optional_field(&map, "repo_id"),
            route_id: gitp2p_metadata::optional_field(&map, "route_id"),
            path: gitp2p_metadata::optional_field(&map, "path"),
            phase: gitp2p_metadata::optional_field(&map, "phase"),
        });
    }
    Ok(paths)
}
    ...
    let sync_target = {
        ...
        if let Some(selected) = select_route(&routes) {
            ...
            app.all_peers()?
                ...
                .map(|peer| peer.id)
                .ok_or_else(|| {
                    gitp2p_metadata::AppError::new(format!(
