use gitp2p_federation::{create_domain, find_domain, list_domains, write_domain};
use gitp2p_metadata::FederationDomain;
use gitp2p_identity::{domain_id, inspect_identity};
use gitp2p_metadata::Result;
use gitp2p_peering::list_peerings;
use gitp2p_trust::sign_bytes;
...
pub fn migrate_domain(
    ...
    let source = list_domains(app)?
        .into_iter()
        .next()
        .ok_or_else(|| gitp2p_metadata::AppError::new("no local domain to migrate"))?;
    ...
    updated.signed_at = gitp2p_metadata::util::timestamp();
