...
use gitp2p_federation::{ensure_federation_layout, exchange_dir, gateways_dir, write_exchange_manifest};
use gitp2p_identity::gateway_id;
use gitp2p_metadata::{
...
pub fn find_gateway(app: &App, reference: &str) -> Result<Gateway> {
    list_gateways(app)?
        ...
        .ok_or_else(|| gitp2p_metadata::AppError::new(format!("gateway '{reference}' not found")))
}
