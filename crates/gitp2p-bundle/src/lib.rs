...
use gitp2p_manifest::{bundle_layout, new_manifest, write_manifest};
use gitp2p_lineage::{build_lineage_chain, lineage_hash};
use gitp2p_metadata::git::git;
use gitp2p_metadata::{optional_field, read_kv, Repo, RepoAction, Result, Vault};
...
pub fn import_bundle(app: &App, bundle: &Path, vault_ref: Option<&str>) -> Result<(Vault, Repo)> {
    ...
    let vault = match vault_ref {
        ...
        None => app
            ...
            .into_iter()
            .next()
            .ok_or_else(|| gitp2p_metadata::AppError::new("no vault exists; create one or pass --vault"))?,
    };
    ...
    if !checkpoint_id.is_empty() && !commit.is_empty() {
        let mut checkpoint = gitp2p_metadata::Checkpoint {
            id: checkpoint_id,
        ...
        if let Ok(identity) = app.ensure_identity() {
            gitp2p_trust::sign_checkpoint(&identity, &mut checkpoint)?;
        }
        let cp_path = vault
            .path
            .join("metadata")
