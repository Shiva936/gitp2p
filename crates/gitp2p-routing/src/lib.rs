...
use gitp2p_identity::federation_route_id;
use gitp2p_metadata::{field, optional_field, read_kv, write_kv, GlobalRoute, Peer, Result};
...
pub fn write_global_route(home: &Path, route: &GlobalRoute) -> Result<()> {
    gitp2p_metadata::util::create_dir_all(global_routing_dir(home))?;
    write_kv(
...
pub fn build_global_route(app: &App, destination: &str) -> Result<GlobalRoute> {
    ...
    let local_domain = gitp2p_federation::local_domain(app)?
        .map(|d| d.id)
...
pub fn verify_route(app: &App, route_id: &str) -> Result<GlobalRoute> {
    let route = inspect_global_routes(app)?
        ...
        .ok_or_else(|| gitp2p_metadata::AppError::new(format!("route '{route_id}' not found")))?;
    if route.state == "revoked" {
        return Err(gitp2p_metadata::AppError::new("route is revoked"));
    }
    let chain = gitp2p_trust::inspect_delegation_chain(&app.home, None)?;
    if !chain.is_empty() {
        let identity = app.ensure_identity()?;
        gitp2p_trust::validate_delegation_chain(&app.home, &identity, &chain)?;
    }
    Ok(route)
}
