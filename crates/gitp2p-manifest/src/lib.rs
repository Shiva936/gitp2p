...
use gitp2p_metadata::{read_kv, write_kv, Result};
...
pub fn read_manifest(path: &Path) -> Result<FederationManifest> {
    ...
    Ok(FederationManifest {
        version: gitp2p_metadata::optional_field(&map, "version"),
        repo_id: gitp2p_metadata::optional_field(&map, "repo_id"),
        checkpoint_id: gitp2p_metadata::optional_field(&map, "checkpoint_id"),
        lineage: gitp2p_metadata::optional_field(&map, "lineage"),
        lineage_hash: gitp2p_metadata::optional_field(&map, "lineage_hash"),
        trust_zone: gitp2p_metadata::optional_field(&map, "trust_zone"),
        created_at: gitp2p_metadata::optional_field(&map, "created_at"),
    })
}

pub fn verify_manifest(path: &Path) -> Result<String> {
    let manifest = read_manifest(path)?;
    let expected = gitp2p_metadata::optional_field(&read_kv(path)?, "manifest_hash");
