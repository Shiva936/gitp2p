...
use gitp2p_manifest::{bundle_layout, new_manifest, write_manifest, FederationManifest};
use gitp2p_metadata::{Result, Vault};
...
pub fn export_vault(app: &App, vault_ref: &str, output: Option<PathBuf>) -> Result<VaultExport> {
    ...
    for repo in app.all_repos()?.into_iter().filter(|r| r.vault_id == vault.id) {
        ...
        let mirror = gitp2p_vault::layout::mirror_path(&vault, &repo);
        gitp2p_metadata::git::git(
    ...
    let manifest = new_manifest(
        ...
        "vault-export",
        &vault.id,
        &gitp2p_metadata::util::stable_id(&vault.id),
        "trusted",
    );
...
pub fn import_vault(app: &App, package: &Path, name: Option<&str>) -> Result<Vault> {
    ...
    let manifest = gitp2p_manifest::read_manifest(&layout.manifest)?;
    gitp2p_manifest::verify_manifest(&layout.manifest)?;
    let vault_name = name.unwrap_or(&manifest.repo_id);
    let vault = gitp2p_vault::create_vault(app, vault_name)?;
    for entry in fs::read_dir(&layout.repository_deltas)? {
        let entry = entry?;
        if entry.path().extension().and_then(|s| s.to_str()) == Some("bundle") {
            gitp2p_bundle::import_bundle(app, &entry.path(), Some(&vault.name))?;
        }
    }
    Ok(vault)
}
