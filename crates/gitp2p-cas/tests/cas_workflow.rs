use gitp2p_cas::{cas_root, chunk_id, store_chunk, verify_chunk};
use gitp2p_dedup::deduplicate_store;
use gitp2p_identity::{peer_id_from_key, vault_id};
...
fn cas_dedup_and_merkle() {
    let home = std::env::temp_dir().join(format!(
        "gitp2p-v45-{}",
        gitp2p_metadata::util::stable_id("cas")
    ));
