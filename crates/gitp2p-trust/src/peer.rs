...
use gitp2p_metadata::{field, optional_field, read_kv, write_kv, Peer, Result};
use gitp2p_metadata::util::timestamp;

use crate::identity::validate_peer_identity;
...
pub fn authorize_peer(peer: &Peer, requires_approval: bool) -> Result<()> {
    ...
    match peer.trust_state.as_str() {
        ...
        state => Err(gitp2p_metadata::AppError::new(format!(
            "peer '{}' is not authorized for synchronization (state: {state}); run trust approve first",
            peer.id
        ))),
    }
}
