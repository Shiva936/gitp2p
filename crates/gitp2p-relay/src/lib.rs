...
pub fn relay_status(app: &App) -> Result<RelayState> {
    ...
    let map = gitp2p_metadata::read_kv(&path)?;
    Ok(RelayState {
        enabled: gitp2p_metadata::optional_field(&map, "enabled") == "true",
        forwarded: gitp2p_metadata::optional_field(&map, "forwarded")
            .parse()
            .unwrap_or(0),
    })
}
...
pub fn forward_propagation(app: &App, artifact: &str, next_hop: &str) -> Result<()> {
    ...
    if !state.enabled {
        return Err(gitp2p_metadata::AppError::new("relay is disabled"));
    }
